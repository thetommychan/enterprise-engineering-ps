...
#$outfile1 = "\\share02\server\powershell\Tom\$username-delprof-$(get-date -f yyyMMdd-HHmmss).log"
...
$richTextBox1.Clear()
 #$outfile1="c:\domain\$username-delprof-$(get-date -f yyyMMdd-HHmmss).log" 
 $outfile1 = "\\share02\helpdesk\delproflog\$username-delprof-$(get-date -f yyyMMdd-HHmmss).log"
...
.DESCRIPTION
    ...
    It uses Active Directory user's name in the form ABCDEFG without prefix (DomainName\) or without suffix (@domain.com)    
    It requires:
    1.NTFSSecurity module (avoid the latest version of it from the internet. Use one from \\share02\Server\scripts\DeleteAppData_PS)
...
.EXAMPLE
   Delete-CompanyProfile -name APC1354
.EXAMPLE
    Delete-CompanyProfile ABC1DEF
#>
[CmdletBinding()]
...
#check the user name and make sure user exists
    ...
    {
        OutputText "User $name does not exist. Check spelling?`rYou have to use Company user name NOT US\ name`r"
        $allisgood = $false
    }
...
# Check for existing sessions in XA7
   If ($AllIsGood)
   {
        ...
        $connecttest13=test-connection CitrixController13 -quiet
        $connecttest14=test-connection CitrixController14 -quiet
        If ($connecttest13)
        {
            $controller="CitrixController13"
        }
            elseif ($connecttest14)
        {
            $controller="CitrixController14"
        }
            else 
        {
    ...
    $sesscount = get-brokersession -adminaddress $controller -brokeringusername DomainName\$nameupper
    ...
    {
        $testOwner = Get-CompanyOwner $name
    ...
    {
        #2021-09-28 -- (WSH) -- Added section to make a backup of the MS Edge Bookmarks and Preferneces files       #  $redir="\\domain.com\FileShare-domain\users\ntst002\redir"
    ...
    function Set-AppDataOld
    {
        ...
        $redir = "\\domain.com\FileShare-domain\ctx_users\$using:Name\REDIR" 
    ...
    function Remove-AppData
    {
        if ($AllIsGood) 
        {
            ...
            $redir = "\\domain.com\FileShare-domain\ctx_users\$Name\REDIR"
            $checkforappdata = Test-Path "$redir\appdata"
            if (!($checkforappdata))
            {
                OutputText "Message: AppData folder at \\domain.com\FileShare-domain\users\$Name\REDIR does not exist.`rProceeding... `r`r"
...
#Step3b delete Citrix Profile unless there are existing sessions (variable $allisGood) -- Backup chrome and MSEdge Bookmark files FIRST
    ...
    If ($AllIsGood)
    {
        ...
        $fldrnames3b=Get-Childitem "\\domain.com\FileShare-domain\xa7x_profiles\" -filter $name* -Directory    #   $fldr3b = "nfon157.v2"
        Foreach ($fldr3b in $fldrnames3b)
        {
            $b=Test-Path "\\domain.com\FileShare-domain\xa7x_profiles\$fldr3b"  
            if ($b=$true)
            {         
                Remove-Item "\\domain.com\FileShare-domain\xa7x_profiles\$fldr3b" -Recurse -Force  -ErrorAction SilentlyContinue  
                $b1=Test-Path "\\domain.com\FileShare-domain\xa7x_profiles\$fldr3b"
                If ($b1=$true)
                {
                    OutputText "Citrix Profile \\domain.com\FileShare-domain\xa7x_profiles\$fldr3b deleted `r"
                }
                    else
                {
                    OutputText "Citrix Profile \\domain.com\FileShare-domain\xa7x_profiles\$fldr3b was NOT deleted.  `r"
                }
            }
                else
            {
                OutputText "WARNING: Citrix XA7 Profile at \\domain.com\FileShare-domain\xa7x_profiles\$fldr3b does not exist.`rThat may not necessarily be a problem.`r"
            }
        }  
